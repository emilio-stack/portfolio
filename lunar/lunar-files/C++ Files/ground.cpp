 ...
 ************************************************************************/
  ...
  ********************************************/
 double Ground::getElevation(const Point& pt, const int width) const
 {
   if (pt.getX() >= 0.0 && pt.getX() < ptUpperRight.getX())
   {
      int minX = max(0, (int)(floor(pt.getX() - width / 2.0)));
      int maxX = min((int)(ptUpperRight.getX()), (int)(ceil(pt.getX() + width / 2.0)));
      int minElevation = pt.getY();
      for (int x = minX; x <= maxX; x++)
      {
         minElevation = min(minElevation, (int)(pt.getY() - ground[x]));
      }
      return minElevation;
   }
   else
...
bool Ground :: onPlatform(const Point & position, int landerWidth) const
{
   ...
   if (getElevation(position, landerWidth) > 1.0)
      return false;

   // not on the platform if we hit the ground
   if (getElevation(position, landerWidth) < 0.0)
      return false;
